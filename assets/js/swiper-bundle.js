(() => {
		...
		off(events2, handler) {
			...
			events2.split(' ').forEach(event2 => {
				if (typeof handler === 'undefined') {
					...
					self.eventsListeners[event2].forEach(
						(eventHandler, index) => {
							if (
			...
			if (shouldSnapToSlideEdge) {
				...
				if (params.slidesPerView === 'auto') {
					...
					for (let i = slidesSizesGrid.length - 1; i >= 0; i -= 1) {
						accumulatedSize +=
							slidesSizesGrid[i] +
							(i < slidesSizesGrid.length - 1 ? spaceBetween : 0);
				...
				lastAllowedSnapIndex = Math.max(
					slidesLength - minVisibleSlides,
					0
				);
			}
			const newSlidesGrid = [];
	...
	function updateProgress(translate2) {
		...
		if (translatesDiff === 0) {
			...
			const isEndRounded =
				Math.abs(translate2 - swiper.maxTranslate()) < 1;
	...
	function slideTo(index = 0, speed, runCallbacks = true, internal, initial) {
		...
		if (speed === 0) {
			...
			if (!swiper.onSlideToWrapperTransitionEnd) {
				swiper.onSlideToWrapperTransitionEnd = function transitionEnd2(
					e
				) {
					if (!swiper || swiper.destroyed) return;
					if (e.target !== this) return;
	...
	function loopFix({
		...
		const activeColIndexWithShift =
			activeColIndex +
			(bothDirections && typeof setTranslate2 === 'undefined'
				? -slidesPerView / 2 + 0.5
				: 0);
	...
	function Autoplay({ swiper, extendParams, on, emit, params }) {
		...
		const pause = (internal, reset) => {
			...
			autoplayTimeLeft =
				delay -
				/* @__PURE__ */ (new Date().getTime() - autoplayStartTime);
