...
function core_prepare_field_group( $data ) {
	...
	if ( ! $fields_integrity_check ) {
		return core_add_notice(
			'warning',
			"Invalid declaration of custom fields: {$data['slug']}",
			core_docs( 'custom-fields' )
		);
	}
...
/**
 * Adds a notice if the user is on an ACF UI type page on the Backend.
 *
 * @since 1.0.0
 * @internal
 */
function core_alert_about_acf_ui() {
	if ( ! is_admin() ) return;
	...
	if (
		$current_screen &&
		in_array( $current_screen->base, [ 'edit', 'post' ] ) &&
		in_array( $current_screen->id, [ 'edit-acf-field-group', 'acf-field-group' ] )
	) {
		core_add_notice( 'alert', "Don't use the ACF UI in production.", [
			"You are free to use the ACF UI in development, for testing purposes, but using it in production is not supported.",
			core_docs( 'custom-fields' )
		]);
	}
}

add_action( 'current_screen', 'core_alert_about_acf_ui' );
...
/**
 * Applies custom filters to the `get_fields` function from ACF.
 *
 * @since 1.1.8
 * @internal
 *
 * @param mixed[] $fields
 * @param string $content_type Can be 'post', 'section' or 'option'.
 * @param int|string [$reference=null] The unique reference. Can be a post ID or a section name.
 */
function core_acf_fields_apply_filters( $fields, $content_type, $reference=null ) {
	...
	if ( $content_type === 'post' ) {
		 ...
		 * @var string
		 */
		$post_type = get_post_type( $reference );
		 ...
		 * @var string|null
		 */
		$page_template = $post_type === 'page' ? get_post_meta( $reference, '_wp_page_template', true ) : null;

		// ? Apply filters: Post Type
		$fields = apply_filters( "core/fields/post-type={$post_type}", $fields );

		// ? Apply filters: Page Template
		if ( $page_template ) {
			$fields = apply_filters( "core/fields/page-template={$page_template}", $fields );
		}
	}
	...
	// * Apply Filters: Section
		...
		// ? Apply filters: Section
		$fields = apply_filters( "core/fields/section={$reference}", $fields );
	}

	// * Apply Filters: Option
	// There are no special filters for global options.
...
/**
 * Retrieves the post fields, applying custom filters.
 *
 * @since 1.1.8
 * @internal
 *
 ...
 * @return array
 */
function core_get_fields( $post_id=null ) {
	...
	/**
	 * The content type, based on the post ID.
	 *
	 * @var string
	 */
	$content_type = $post_id === 'option' ? 'option' : 'post';

	// Apply filters.
	$fields = core_acf_fields_apply_filters( $fields, $content_type, $post_id );
...
require_once CORE_PATH . '/acf/custom-fields-format.php';
