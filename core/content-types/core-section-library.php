...
function core_scan_sections() {
	...
	$sections_list = core_scan_dir( PKG_CONTENT_PATH.'/section-library', 'config.php', function( $file ) {
		...
		try {
			...
			$reg_data['includes_preview_image'] = is_file( "{$reg_data['location']}/preview.jpeg" );
			$reg_data['multiple'] = core_default( 'multiple', $reg_data, true );
			$reg_data['node'] = core_default( 'node', $reg_data, [] );

			// Check if the section was added to a category.
			if ( count( explode( '/', $reg_data['location_rel'] ) ) !== 3 ) {
				core_add_notice( 'error', 'Invalid section library structure.', [
					'Sections must be placed inside category-defining directories.',
					"Section: `{$reg_data['slug']}`",
					core_docs( 'section-library' )
				]);

				return null;
			} else {
				$reg_data['category'] = explode( '/', $reg_data['location_rel'] )[1];
			}
			...
			// Scan partials.
			$reg_data['partials'] = core_scan_partials( $file['info']['dirname'] );
			...
			if ( is_file( $reg_data['location'].'/filters.php' ) ) {
				...
				if ( $filters && core_is_assoc( $filters ) ) {
					foreach ( $filters as $filter_type => $filter_func ) {
						add_filter( "core/{$filter_type}/section={$reg_data['slug']}", $filter_func, 10 );
					}
				}
			}
	...
	$missing_previews = array_filter( $sections_list, function( $section ) {
...
/**
 * Implements additional categories for sections in the backend.
 *
 * @since 2.11.1
 * @internal
 *
 * @return array[]
 */
function core_add_section_categories() {
	$raw_categories = array_values(
		array_filter( scandir( PKG_CONTENT_PATH . '/section-library' ), function( $file ) {
			return ! in_array( $file, [ '.', '..' ] ) && strpos( $file, '.' ) === false;
		})
	);
...
add_filter( 'block_categories_all', 'core_add_section_categories' );

/**
 * Adds a custom function to Twig that lets you render section library fields.
 *
 * @since 1.0.0
 * @since 1.1.2  Added unique ID implementation.
 * @since 2.11.0 Replaced `section_library` with `post_sections`, which renders
 *               the Block Editor, instead of Flexible Content layouts.
 * @internal
 *
 * @param Twig $twig
 * @return Twig
 */
function core_twig_add_section_library( $twig ) {

	// Render the post sections.
	$twig->addFunction(
		new Twig\TwigFunction( 'post_sections', function() {
			$sections = core_get_post_sections();
			if ( ! $sections || empty( $sections ) ) return;

			echo '<div class="core-section-library-wrapper">';

			foreach ( $sections as $section ) {
				echo render_block( $section );
			}
	...
	// Render a standalone section.
	$twig->addFunction(
		new Twig\TwigFunction( 'section', function( $section_fields = [] ) {
			if ( ! is_array( $section_fields ) ) return;

			$section_slug = core_default( 'slug', $section_fields, false );
			if ( ! $section_slug ) return;
			unset( $section_fields['slug'] );

			// Render the section.
			echo '<div class="core-section-library-wrapper">';
			core_render_section( $section_slug, $section_fields );
			echo '</div>';
		})
	);
...
add_filter( 'timber/twig', 'core_twig_add_section_library' );

/**
 * Generates a preview file for the specified section.
 *
 * @since 2.11.0
 */
function core_generate_section_preview_file() {
	$env = core_env();
	if ( $env['current'] === 'prod' ) return;
	...
	$section = core_get_section( $section_slug );
	if ( ! $section ) die();

	// Generate the file.
	$file_path = "{$section['location']}/preview.jpeg";
	if ( is_file( $file_path ) ) unlink( $file_path );

	$file = fopen( $file_path, 'wb' );
	fwrite( $file, base64_decode( str_replace( 'data:image/jpeg;base64,', '', $image_data ) ) );
	fclose( $file );

	die();
}

add_action( 'wp_ajax_generate_section_preview', 'core_generate_section_preview_file' );
