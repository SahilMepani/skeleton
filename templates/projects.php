...
<div class="container padding-4">
	...
	<div class="project-filters-block margin-bottom-3">
		...
		<div class="custom-select-block hidden-sm hidden-md hidden-lg">
			<label for="ajax-first-select-post-categories" class="sr-only"><?php _e( 'Select Tax One', 'skel' );?></label>
			<select id="ajax-first-select-post-categories">
				<option data-cpt="project" data-first-cpt-tax="tax-one" data-first-cat-id="-1">
					<?php _e( 'All Tax One', 'skel' );?></option>
				<?php
					$cats_args = [
						'taxonomy' => 'tax-one'
					];
					...
					foreach ( $cats as $cat ):
				?>
				<option data-cpt="project" data-first-cpt-tax="tax-one"
					data-first-cat-id="<?php echo $cat->term_id; ?>">
					<?php echo $cat->name; ?>
				</option>
				<?php endforeach;?>
			</select>
		...
		<div class="custom-select-block hidden-sm hidden-md hidden-lg">
			<label for="ajax-second-select-post-categories"
				class="sr-only"><?php _e( 'Select Tax Two', 'skel' );?></label>
			<select id="ajax-second-select-post-categories">
				<option data-cpt="project" data-second-cpt-tax="tax-two" data-second-cat-id="-1">
					<?php _e( 'All Tax Two', 'skel' );?></option>
				<?php
					$cats_args = [
						'taxonomy' => 'tax-two'
					];
					...
					foreach ( $cats as $cat ):
				?>
				<option data-cpt="project" data-second-cpt-tax="tax-two"
					data-second-cat-id="<?php echo $cat->term_id; ?>">
					<?php echo $cat->name; ?>
				</option>
				<?php endforeach;?>
			</select>
		...
		<ul id="ajax-first-list-post-categories" class="list-tax-one-filters list-filters hidden-xs">
			<li class="js-active">
				<a href="javascript:void(0);" data-cpt="project" data-first-cpt-tax="tax-one"
					data-first-cat-id="-1"><?php _e( 'ALL', 'skel' );?></a>
			</li>
			<?php
				$cats_args = [
					'taxonomy' => 'tax-one'
			...
			<li>
				<a href="javascript:void(0);" data-cpt="project" data-first-cpt-tax="tax-one"
					data-first-cat-id="<?php echo $cat->term_id; ?>"><?php echo $cat->name; ?></a>
			</li>
			<?php }?>
		...
		<ul id="ajax-second-list-post-categories" class="list-tax-two-filters list-filters hidden-xs">
			<li class="js-active">
				<a href="javascript:void(0);" data-cpt="project" data-second-cpt-tax="tax-two"
					data-second-cat-id="-1"><?php _e( 'ALL', 'skel' );?></a>
			</li>
			<?php
				$cats_args = [
					'taxonomy' => 'tax-two'
			...
			<li>
				<a href="javascript:void(0);" data-cpt="project" data-second-cpt-tax="tax-two"
					data-second-cat-id="<?php echo $cat->term_id; ?>"><?php echo $cat->name; ?></a>
			</li>
			<?php }?>
	...
	<div class="text-center clear">
		...
		<div class="btn-load-more-block clear">
			<button id="ajax-load-more-post-dual" data-cpt="project" data-first-cpt-tax="tax-one"
				data-second-cpt-tax="tax-two"
				class="<?php echo ( $post_count < 6 ) ? 'disabled' : 'js-active'; ?> btn btn-black btn-lg btn-load-more"><b><?php _e( 'LOAD MORE', 'skel' );?></b></button>
		</div>
	</div> <!-- .text-center -->
